...
from estrellas.herramientas_legacy import HOMIEN, SIGRMI, UNANGGRA, HOMI
# AÑADIMOS 'aplicar_delta_t_manual' A LA IMPORTACIÓN
from estrellas.calculos import cargar_catalogo, calcular_posicion_aparente, calcular_paso_meridiano_greenwich, aplicar_delta_t_manual
...
def main():
    ...
    for i in range(len(ll)):
        if ll[i]: ll[i] = ll[i].ljust(80) 
    for i in range(len(l)):
        if l[i]: l[i] = l[i].ljust(70)   
    ...
    for n in range(num_estrellas):
        ...
        for k in range(12):
            ...
            if modo == 1:
                _, al_hora = calcular_paso_meridiano_greenwich(jd, estrella.skyfield_star)
                hh, mm = HOMIEN(al_hora)
                gr1[k] = hh
                mi1[k] = mm
            else:
                ra_rad, dec_rad = calcular_posicion_aparente(jd, estrella.skyfield_star)
                al_SHA = (2.0 * PI) - ra_rad
                if al_SHA < 0: al_SHA += 2.0*PI
        ...
        if modo == 1:
            ...
            ra_dummy, dec_dummy = calcular_posicion_aparente(dia_juliano_simple(dia_mes, 1, ano), estrella.skyfield_star)
    ...
    if modo == 1:
        ...
        def parse_w_robust(w_str):
            ...
            if de_index >= 0 and de_index + 2 < len(parts_w):
                ...
                if len(minutos_de) < 12: minutos_de += ['0.0'] * (12 - len(minutos_de))
        ...
        f376.close(); f377.close(); f378.close(); f379.close(); fcar.close(); fcde.close()
